...
function readFormula(fileName) {
  const fs = require('fs')
  let textlinha = fs.readFileSync('//virtualdisk.cin.ufpe.br/laapb/Projeto IC/satmaster/' + fileName, 'utf8')
...
function doSolve(arrayTestes, clausulas) {
  let stop = false;
  for (let contadortestes = 0; contadortestes < arrayTestes.length && stop == false; contadortestes++) {
    //testes mandados
    for (let contadorclausulas = 0; contadorclausulas < clausulas.length && stop == false; contadorclausulas++) {
      ...
      if (doTest(arrayTestes[contadortestes], clausulas[contadorclausulas]) == false) {
        ...
        return { boolean: true, teste: arrayTestes[contadortestes] };
      }
    }
  }
  if(stop==true){
